...
void BuildTrianglesAndAddToVirtualScene(std::map<std::string, SceneObject>& scene, ObjModel* model)
{
    ...
    for (size_t shape = 0; shape < model->shapes.size(); ++shape)
    {
        ...
        scene[model->shapes[shape].name] = theobject;
    }
...
void LoadShadersFromFiles(ShaderProgram& shader)
{
    ...
    shader.programId = CreateGpuProgram(vertex_shader_id, fragment_shader_id);

    // Buscamos o endereço das variáveis definidas dentro do Vertex Shader.
    // Utilizaremos estas variáveis para enviar dados para a placa de vídeo
    // (GPU)! Veja arquivo "shader_vertex.glsl" e "shader_fragment.glsl".
    shader.model_uniform      = glGetUniformLocation(shader.programId, "model"); // Variável da matriz "model"
    shader.view_uniform       = glGetUniformLocation(shader.programId, "view"); // Variável da matriz "view" em shader_vertex.glsl
    shader.projection_uniform = glGetUniformLocation(shader.programId, "projection"); // Variável da matriz "projection" em shader_vertex.glsl
    shader.object_id_uniform  = glGetUniformLocation(shader.programId, "object_id"); // Variável "object_id" em shader_fragment.glsl
    shader.bbox_min_uniform   = glGetUniformLocation(shader.programId, "bbox_min");
    shader.bbox_max_uniform   = glGetUniformLocation(shader.programId, "bbox_max");

    // Variáveis em "shader_fragment.glsl" para acesso das imagens de textura
    glUseProgram(shader.programId);
    glUniform1i(glGetUniformLocation(shader.programId, "TextureImage0"), 0);
    glUniform1i(glGetUniformLocation(shader.programId, "TextureImage1"), 1);
    glUniform1i(glGetUniformLocation(shader.programId, "TextureImage2"), 2);
    glUniform1i(glGetUniformLocation(shader.programId, "TextureImage3"), 3);

    glUniform1i(glGetUniformLocation(shader.programId, "TextureImage4"), 4);
    glUniform1i(glGetUniformLocation(shader.programId, "TextureImage5"), 5);
    glUniform1i(glGetUniformLocation(shader.programId, "TextureImage6"), 6);
    glUniform1i(glGetUniformLocation(shader.programId, "TextureImage7"), 7);
    glUniform1i(glGetUniformLocation(shader.programId, "TextureImage8"), 8);
...
{
    // Check if object exists to avoid crash
    if (scene.find(object_name) == scene.end()) {
        // fprintf(stderr, "Warning: Object '%s' not found in scene.\n", object_name);
        return;
    }

    const SceneObject& obj = scene.at(object_name);

    glBindVertexArray(obj.vertex_array_object_id);

    glm::vec3 bbox_min = obj.bbox_min;
    glm::vec3 bbox_max = obj.bbox_max;
    glUniform4f(shader.bbox_min_uniform, bbox_min.x, bbox_min.y, bbox_min.z, 1.0f);
    glUniform4f(shader.bbox_max_uniform, bbox_max.x, bbox_max.y, bbox_max.z, 1.0f);
...
void DrawDebugSphere(const Sphere& s, glm::mat4 view, glm::mat4 projection, const ShaderProgram& shader, const std::map<std::string, SceneObject>& scene)
{
    ...
    DrawVirtualObject(scene, "the_sphere", shader);
...
void DrawDebugBox(const AABB& box, glm::mat4 view, glm::mat4 projection, const ShaderProgram& shader, const std::map<std::string, SceneObject>& scene)
{
    ...
    DrawVirtualObject(scene, "the_cube", shader);
...
void DrawAllObjects(const std::vector<std::tuple<glm::mat4, const char*, int>>& objects, const std::map<std::string, SceneObject>& scene, const ShaderProgram& shader){
    for (auto e : objects){
        glUniformMatrix4fv(shader.model_uniform, 1 , GL_FALSE , glm::value_ptr(std::get<0>(e)));
        glUniform1i(shader.object_id_uniform, std::get<2>(e));
        DrawVirtualObject(scene, std::get<1>(e), shader);
    }
}